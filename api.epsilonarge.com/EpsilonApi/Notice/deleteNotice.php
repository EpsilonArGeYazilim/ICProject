...
if(isset($_GET["key"])){
    ...
    if(isset($postdata)){
        ...
        if (!empty($request->user_id) && !empty($request->user_token)) {
            ...
            $notice = new Notice();
            echo json_encode($notice->deleteNotice($notice_id,$user_id, null, $user_token, null), JSON_UNESCAPED_UNICODE);
    } else if (!empty($request->admin_id) && !empty($request->admin_token)) {
            $admin_id = $request->admin_id;
            $admin_token = $request->admin_token;
            $notice = new Notice();
            echo json_encode($notice->deleteNotice($notice_id,null, $admin_id, null, $admin_token), JSON_UNESCAPED_UNICODE);
    } else {
        $result["result"] = false;
        $result["code"] = 1001;
